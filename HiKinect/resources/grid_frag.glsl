...
void main()
{
	...
	vec4 normalMap = texture2D(normalMap, gl_TexCoord[0].xy);
	vec2 colorUV = vec2( gl_TexCoord[0].x, 1.0 - gl_TexCoord[0].y );
	vec4 colorMap = texture2D( colorMap, colorUV );
	
	gl_FragColor.rgb = color.rgb * vertColor.rgb * colorMap.rgb;
	gl_FragColor.a = 1.0;
}
